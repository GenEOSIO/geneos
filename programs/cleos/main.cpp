...
int main( int argc, char** argv ) {
   ...
   app.set_callback([] { ensure_kgeneosd_running();});
...
//                                ->check(CLI::ExistingFile);
   ...
   auto stopkgeneosd = wallet->add_subcommand("stop", localized("Stop kgeneosd (doesn't work with nodgeneos)."), false);
   stopkgeneosd->set_callback([] {
      // wait for kgeneosd to come up
      try_port(uint16_t(std::stoi(parse_url(wallet_url).port)), 2000);
