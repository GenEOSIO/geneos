...
getHeadBlockNum()
{
  INFO="$(programs/clgeneos/clgeneos get info)"
  verifyErrorCode "clgeneos get info"
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clgeneos/clgeneos --port $port2 get block 1)"
    verifyErrorCode "clgeneos get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clgeneos/clgeneos create key)"
verifyErrorCode "clgeneos create key"
...
PASSWORD_INITA="$(programs/clgeneos/clgeneos wallet create --name inita)"
verifyErrorCode "clgeneos wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clgeneos/clgeneos wallet import --name inita $INITA_PRV_KEY
verifyErrorCode "clgeneos wallet import"
programs/clgeneos/clgeneos wallet import --name inita $PRV_KEY1
verifyErrorCode "clgeneos wallet import"
programs/clgeneos/clgeneos wallet import --name inita $PRV_KEY2
verifyErrorCode "clgeneos wallet import"
programs/clgeneos/clgeneos wallet import --name inita $PRV_KEY3
verifyErrorCode "clgeneos wallet import"
...
ACCOUNT_INFO="$(programs/clgeneos/clgeneos create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clgeneos create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clgeneos/clgeneos get account testera)"
verifyErrorCode "clgeneos get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clgeneos/clgeneos transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clgeneos transfer"
   ...
   TRANS_INFO="$(programs/clgeneos/clgeneos --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clgeneos get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
