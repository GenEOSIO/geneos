...
def monitor_exchange():
    ...
    results = clgeneos('get actions exchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' EOS':
        quantity += ' EOS'
    results = clgeneos('transfer exchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
